...
##-----------------------------------------
## historicMoovs, historicMoovsEvents, historicMoovsSteps!!!!!
##-----------------------------------------
...
if not os.path.exists(file_path):
    ...
    df.to_json('External_data/historicMoovs_clean.json')

# Clearing the content.db database and then restarting the connection

conn = sqlite3.connect("content.db")
conn.execute("DELETE FROM historicMoovs;")
conn.execute("DELETE FROM historicMoovsEvents;")
conn.execute("DELETE FROM historicMoovsSteps;")
conn.commit()
conn.close()
...
# query is used to generate the query for historicMoovs table
# queryMessageParams is used to generate the query for messageParams table
query = "INSERT INTO historicMoovs ("
queryEvent = "INSERT INTO historicMoovsEvents (id," 
queryStep = "INSERT INTO historicMoovsSteps ("
idCount = 0
...
conn.commit()
print("historicMoovs Values inserted successfully :)")
print("historicMoovsEvents Values inserted successfully :)")
print("historicMoovsSteps Values inserted successfully :)")
conn.close()
